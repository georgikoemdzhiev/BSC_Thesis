...
The design process has gone through many iterations to ensure the best possible quality of the product. The research presented in chapter \ref{Chapter:Literature-Review} has affected immensely the design, especially section \ref{section:commercial-har-systems}. The design can be split into four main sections, namely Mobile Platform and IDE; Database Design; Architectural design and User Interface (UI).
    ...
    \begin{figure}[H]
        ...
        \begin{tikzpicture}
            ...
            \node at (4.7,1) {Data Preprocessing};
            \draw [->] (6.5,1) -- (7.5,1);
            \draw (7.5,0) rectangle (11,2);
            \node at (9.3,1.5) {Feature Extraction};
            \node at (9.3,1) {and};
            \node at (9.3,0.5) {Feature Selection};
            \draw [->] (11,1) -- (12,1);
            \draw (12,0) rectangle (14,2);
            \node at (13,1) {Classifier};
        \end{tikzpicture}
    ...
    The data processing stage of the \gls{har} process normally involves segmenting the data into time windows. Considering the fact that the system will be running on a mobile device with constraint resources. The segmentation of the data has to be picked carefully. For example, windowing the data at 20 seconds will cause the CPU to do more computations to abstract the data into feature vectors. As it was found in the literature review (see section \ref{subsubsection:learning_alg_accuracy_power_consm}) 3 second time-window was found to produce good results in terms of power consumption and thus the proposed application will employ this window length as well.
