...
The design process has gone through many iterations to ensure the best possible quality of the product. The research presented in chapter \ref{Chapter:Literature-Review} has affected immensely the design, especially section \ref{section:commercial-har-systems}. The design can be split into four main sections, namely Mobile Platform and IDE; Database Design; Architectural design and User Interface (UI).
    ...
    \end{figure}
        ...
        \subsection{Views}
        The View layer of the \gls{mvp} pattern is comprised of Interface classes that are implemented by Android components such as \textit{"Activity"} or \textit{"Fragment"}. The \textit{Activity} class creates a window (e.g. single screen) so the developer can set a specific layout (a specific arangement of \gls{ui} components such as \textit{"Buttons"} or \textit{"TextViews"}) via using the method \textit{"setContentView(View)"} \citep{googleactivity2017}. The \textit{"Fragment"} class can be interpreted as a \textit{"sub-activity"} that is highly reusable and must by hosted (or presented) in an Activity class \citep{googlefragments2017}. For example, the \textit{"LoginFragment"} and \textit{"SignupFragment"} are hosted in an Activity called \textit{"AuthenticationActivity"}. 
