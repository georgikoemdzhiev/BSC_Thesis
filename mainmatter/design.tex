...
The design process has gone through many iterations to ensure the best possible quality of the product. The research presented in chapter \ref{Chapter:Literature-Review} has affected immensely the design, especially section \ref{section:commercial-har-systems}. The design can be split into four main sections, namely Mobile Platform and IDE; Database Design; Architectural design and User Interface (UI).
    ...
    \begin{figure}[H]
        ...
        \begin{tikzpicture}
            ...
            \node at (1.2,1.2) {Accelerometer};
            \node at (1.2,0.8) {Sensor};
            \draw [->] (2.4,1) -- (3.4,1);
    ...
    The data processing stage of \gls{har} normally involves segmenting the data into time windows. Considering the fact that the system will be running on a mobile device with limited resources. The segmentation of the data has to be picked carefully. For example, choosing a 20-second window length will cause the CPU to do more computations to extract data features. As it was found in the literature review (see section \ref{subsubsection:learning_alg_accuracy_power_consm}), 3-second time window was found to produce good results in terms of power consumption, and thus the proposed application will adopt this window length as well.
    
    During this stage, different filters could be applied to the raw data to remove unwanted information. For example, according to \citet[]{googlemotionsensors}, the raw data from an accelerometer sensor contains the earth's gravity forece as well. This factor will cause the device to read magnitude of g = 9.81 m/s2 when it is not accelerating (i.e. laying down on a table). That is why a low pass filter is applied to the raw to remove the force of gravity from the sensor readings. In addition to applying a low pass filter, the magnitude of x,y and z is calculated and used as a 4th dimension (in addition to x,y and z reading from the sensor) as it has been used in previous works to reduce the effect of device orientation \citep[153]{torreshuitzil2015b}.
