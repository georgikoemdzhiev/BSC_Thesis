...
\section{The mobile application}
    ...
    As discussed before, the implementation of the mobile application is following the \gls{mvp} software pattern, which allows for the clear separation of code concepts (see section \ref{section:architectural-design}). To satisfy the non-functional requirements discussed in section \ref{section:system-quality-attributes}, \gls{mvp} is combined with a Dependency Injection principle. Describing how this principle work could fill an article on its own, thus this description will be very brief.
