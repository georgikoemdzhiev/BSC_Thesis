...
This section will discuss the main and most interesting building blocks of the proposed software. First, the realisation of the database design will be addressed. Next, the \textit{ActiveMinutes Service} functionality will be discussed followed by the Application UI.  
   ...
   public Instances getTrainingData(int userId) {
    ...
    As it was mentioned in section \ref{section:active-minutes-service}, this Android service continuously collects contextual information about the logged-in user. In order to do that it requires the functions of two systems discussed earlier in this chapter, namely \textit{HAR} and \textit{Self-Management}. \textit{ActiveMinutesService} utilises these systems so they work together. A logical view of how this is achieved can be seen in figure \ref{fig:active-minutes-service-class-diagram}. 
    ...
    Implementing \textit{HarManager} as \textbf{Observable} and \textit{ActivityManager} as \textbf{Observer}  (refer back to figure \ref{fig:har_system_impl_class_diagram} and \ref{fig:self_management_system}) allows to connect the two systems so they can work together and exchange information in the service. For example, when the service is started (e.g., when a user logs is) it calls a method called \textit{onCreate()} which consequently adds \textit{ActivityManager} to the list of observers of \textit{HarManager} (see listing \ref{observer-observable-example}).
    
    \begin{lstlisting}[caption= Observer-Observable pattern implementation in ActiveMinutesService,
    frame=tlrbr,
    basicstyle=\small,
    captionpos=b,
    ...
    public void onCreate() {
        ...
        satisfyDependencies();
        ...
        mHarManager.addObserver(mActivityMonitor);
    }
