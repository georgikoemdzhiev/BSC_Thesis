...
\end{figure}
    ...
    One of the most important methods of the class is called \textbf{monitorActivity(activityClass: int)}. This method is called by the \textbf{HarManager} mentioned above as soon as an activity type is recognised (every 3 seconds). It passes the recognised activity to the ActivityMonitor via the method mentioned above. ActivityMonitor then is able to react on the activity type. For example, if the activity type is of type \textit{Static}, ActivityMonitor will calls the DataManager's \textit{incCurrentInacInterval()} method which increments the current inactivity interval in the database.
    
    \subsubsection{Frequent positions changes challenge}
    One problem that had to be solved during the implementation of this class was dealing with the frequent changes of the device position. For example if the user pulls out their phone (i.e. to check for missed calls) and put it back in the pocket these series of events are usually interpreted by the HarManager as activity class of type Active (e.g. running, walking or cycling). That would not be a problem if the user is actually being active at the time. However, if the user is being static (e.g. laying on a bed) this becomes problematic as incorrect data will be saved in the database. Without applying any kind of data filtering ActivityMonitor will increment the user's active time incorrectly leading to wrong activity levels data and eventually misleading the user.
    
    After series of experiments it was found that by storing the last 3 recognised activities in a list (i.e. walking, running, walking) and checking if the currently recognised activity is of type \textit{Active}, these frequent device changes are dramatically avoided. That is because, in order for the user's activity to be classified as \textit{Active} they need to put more effort e.g. being active for at least 9 seconds (since every activity is recognised every 3 seconds and we are storing the last 3 activity types). Pragmatically, this is achieved by a method called \textit{applyCorrection()}. The interested reader can find the implementation of \textbf{ActivityMonitor} class in Appendix \ref{chapter:activity-monitor-class}.
